...
%---------------------------------------------------------
  \cventry
  {Geospatial Data Wrangler} % Job title
  {\href{https://hum.ai}{Hum}} % Organization
  {} % Location
  {July 2025 -} % Date(s)
  {
    \begin{cvitems}
      \item {
          Building a ``data engine'' to gather and unify observations of the Earth from
          open and commercial data sources as tokens for training a geospatial
          transformer-based foundation model.
      }
      \item {
          Creating an ergonomic ``model factory'' framework to execute configurable and repeatable
          distributed training runs for arbitrary machine learning models.
      }
    \end{cvitems}
  }
